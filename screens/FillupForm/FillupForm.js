...
const FillupForm = ({ navigation: { setOptions }, route }) => {
    ...
    const submitButton = React.useMemo((handleSubmit, disabled) => () => (<Button disabled={disabled} label="Save Fill-up" onPress={handleSubmit}/>), []);
    const submitForm = React.useCallback((values) => { console.log(values); }, []);
    return (<Formik initialValues={initialValues} onSubmit={(values) => submitForm(values)} validationSchema={FillupSchema}>
      {({ handleSubmit, values }) => {
        ...
        return (<Wrapper>
            <ScrollView>
              ...
              <TextField keyboardType="numeric" label="Gallons" name="fillupGallons"/>
              <TextField keyboardType="numeric" label="Total" name="fillupTotal"/>
              <TextField keyboardType="numeric" label="Odometer" name="fillupOdometer"/>
...
//# sourceMappingURL=FillupForm.js.map