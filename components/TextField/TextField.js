...
const TextField = (_a) => {
    var { label, multiline = false, name, numberOfLines = 1 } = _a, otherProps = __rest(_a, ["label", "multiline", "name", "numberOfLines"]);
    ...
    const inputStyles = React.useMemo(() => {
        ...
        return (Object.assign(Object.assign({}, uniStyles.inputStyle), { alignItems: hasMultipleLines ? 'flex-start' : 'center', color: bgContrast, height: hasMultipleLines ? calcHeight : 'auto' }));
    }, [bgContrast, hasMultipleLines, numberOfLines]);
    const labelStyle = React.useMemo(() => (Object.assign(Object.assign({}, uniStyles.inputLabelText), { color: borderAndTextColor })), [borderAndTextColor]);
    ...
    const FieldIcon = React.useMemo(() => {
        if (statusIcon === '')
            return null;
        return (<View style={uniStyles.fieldIconStyle}>
          <MCIcon color={borderAndTextColor} name={statusIcon} size={25}/>
        </View>);
    }, [borderAndTextColor, statusIcon]);
    return (<View style={styles.wrapStyle}>
      <View style={fieldStyle}>
        <View pointerEvents="none" style={labelWrapStyle}>
          <Text style={labelStyle}>{label}</Text>
        </View>
        <TextInput {...otherProps} multiline={multiline} numberOfLines={numberOfLines} onBlur={handleBlur} onChangeText={field.onChange(name)} onFocus={handleFocus} style={inputStyles} value={R.prop('value', field)}/>
        {FieldIcon}
      </View>
